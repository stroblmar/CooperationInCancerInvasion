...
<<<<<<< HEAD
        ...
        if stabiliseSpatDiscB:
            self.SpatiallyDiscretisedSystem = self.SpatiallyDiscretisedSystem_Stabilised
        else:
...
<<<<<<< HEAD
    ...
    def SpatiallyDiscretisedSystem_Stabilised(self, t, uVec):
        currSVec, currTAVec, currTMVec, currAVec, currMVec = [uVec[k * self.nGrdPts:(k + 1) * self.nGrdPts] for k in
                                                              range(5)]
        # Spatial ODEs for the stroma, S
        dudtVec_S = self.f_stroma(currSVec, currAVec) * (currSVec >= 0) - max([self.dS, 1.]) * currSVec * (currSVec < 0)
        # ODEs from spatially discretised PDE for TA
        dudtVec_TA = self.f_tumourA(currSVec, currTAVec, currTMVec) * (currTAVec >= 0) - max(
            [self.rT, 1.]) * currTAVec * (
                             currTAVec < 0) + \
                     self.InhomogeneousDiffusionOperator(currTAVec, self.DT * (1 - currMVec / (1 + self.eps)))
        # ODEs from spatially discretised PDE for TM
        dudtVec_TM = self.f_tumourM(currSVec, currTAVec, currTMVec) * (currTMVec >= 0) - max(
            [self.rT, 1.]) * currTMVec * (
                             currTMVec < 0) + \
                     self.InhomogeneousDiffusionOperator(currTMVec, self.DT * (1 - currMVec / (1 + self.eps)))
        # ODEs from spatially discretised PDE for A
        dudtVec_A = self.f_acid(currTAVec, currAVec) * (currAVec >= 0) - max([self.rL, 1.]) * currAVec * (
                currAVec < 0) + \
                    self.InhomogeneousDiffusionOperator(currAVec, np.ones_like(currAVec))

        # Spatial  ODEs for the matrix, M
        dudtVec_M = self.f_matrix(currTMVec, currMVec) * (currMVec >= 0) - max([self.dM, 1.]) * currMVec * (
                currMVec < 0)
...
<<<<<<< HEAD
    ...
    # ============================================================
    # Function to plot the state of the system at a set of points in time
    def PlotDiagnosticPlots(self, figsize=None, saveFigB=False, outName=None):
        if figsize != None: plt.figure(figsize=figsize)
        plt.subplot(5, 1, 1)
        plt.plot(self.loggedTimeVec, self.S.min(axis=0), color='red' if np.any(self.S < 0) else 'blue')
        plt.xlabel("t")
        plt.ylabel("min(S(t))")
        plt.title("S")

        plt.subplot(5, 1, 2)
        plt.plot(self.loggedTimeVec, self.TA.min(axis=0), color='red' if np.any(self.TA < 0) else 'blue')
        plt.xlabel("t")
        plt.ylabel("min(TA(t))")
        plt.title("TA")

        plt.subplot(5, 1, 3)
        plt.plot(self.loggedTimeVec, self.TM.min(axis=0), color='red' if np.any(self.TM < 0) else 'blue')
        plt.xlabel("t")
        plt.ylabel("min(TM(t))")
        plt.title("TM")

        plt.subplot(5, 1, 4)
        plt.plot(self.loggedTimeVec, self.A.min(axis=0), color='red' if np.any(self.A < 0) else 'blue')
        plt.xlabel("t")
        plt.ylabel("min(A(t))")
        plt.title("A")

        plt.subplot(5, 1, 5)
        plt.plot(self.loggedTimeVec, self.M.min(axis=0), color='red' if np.any(self.M < 0) else 'blue')
        plt.xlabel("t")
        plt.ylabel("min(M(t))")
        plt.title("M")
